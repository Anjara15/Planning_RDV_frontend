...
const loadAppointmentsFromStorage = () => {
  try {
    ...
    // Normaliser les données pour avoir un format uniforme
    return appointments.map(appointment => {
      // Parser le username s'il est en JSON string
      let userInfo = {};
      try {
        if (typeof appointment.username === 'string' && appointment.username.startsWith('{')) {
      ...
      return {
        ...
        patientName: appointment.nom && appointment.prenom 
          ? `${appointment.prenom} ${appointment.nom}` 
          : userInfo.username || 'N/A',
        patientPhone: appointment.telephone || 'N/A',
        patientEmail: appointment.email || userInfo.email || 'N/A',
        doctorName: appointment.medecin || 'Médecin généraliste',
        specialty: appointment.specialite || 'Généraliste',
        type: appointment.demande || 'Consultation',
        status: status,
